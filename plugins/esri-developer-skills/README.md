...
The package also includes the `esri-developer` MCP server configuration and the Copilot-specific Esri Developer agent. Skills are discovered from the fixed `skills/` directory; the MCP server is discovered from the package-root `mcp.json`.
